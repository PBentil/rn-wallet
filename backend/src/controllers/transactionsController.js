...
export async function getTransactions(req , res){
    try {
        const { id } = req.params;
        console.log("Looking for user_id:", id);
...
export async function transactionSummary(req, res){
    try{
        ...
        const expenseResult = await sql`SELECT COALESCE(SUM(amount), 0) AS expenses FROM transactions WHERE user_id = ${id} AND amount < 0`;
        res.status(200).json({
